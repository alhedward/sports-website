#!/usr/bin/env python3
"""
LROC/SGARS Deployment Helper v1.14
...
- Finds lroc/ or sgars/ inside the package.
...
APP_TITLE = "LROC/SGARS Deployment Helper"
...
# Package deploy script. Place this as lroc/file.deploy.txt or sgars/file.deploy.txt.
...
if command -v terraform >/dev/null 2>&1 && [ -d "{{SITE_NAME}}/terraform" ]; then cd "{{DEST_PARENT_DIR}}/{{SITE_NAME}}/terraform" && terraform fmt && cd "{{DEST_PARENT_DIR}}"; fi
...
class DeployApp:
    def __init__(self, root: Tk) -> None:
        ...
        self.site_name = StringVar(value="lroc")
    ...
    def build_ui(self) -> None:
        ...
        combo = ttk.Combobox(top, textvariable=self.site_name, values=["lroc", "sgars"], width=10, state="readonly")
    ...
    def extract_package(self) -> None:
        ...
        if not candidates:
            candidates = [p for p in self.extract_dir.iterdir() if p.is_dir() and p.name in {"lroc", "sgars"}]
        if not candidates:
            raise FileNotFoundError("Could not find lroc/ or sgars/ folder inside package.")
